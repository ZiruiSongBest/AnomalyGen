...
Task Name:  Put a knife inside a box
Description: The robotic arm will open a box, grasp the dangerous knife and put it inside the box.
Initial config:
```yaml
-  use_table: True 
-   center: (0.2, 0.3, 0)
...
substep 5: grasp the chef knife
```primitive
	rgbs, final_state = grasp_object(self, "knife")
    success = check_grasped(self, "knife")
```

substep 6: put the chef knife into the box
```reward
def _compute_reward(self):
    # This reward encourages the end-effector to stay near the knife to grasp it.
    knife_position = get_position(self, "knife")
    eef_pos = get_eef_pos(self)[0]
    reward_near = -np.linalg.norm(eef_pos - knife_position)

    # main reward is 1 if the knife is inside the box. From the semantics we know that link2 is the box body
    box_bbox_low, box_bbox_high = get_bounding_box_link(self, "box", "link_2")
    reward_in = int(in_bbox(self, knife_position, box_bbox_low, box_bbox_high))
    
    # another reward is to encourage the robot to move the knife to be near the box
    reward_reaching = - np.linalg.norm(knife_position - (box_bbox_low + box_bbox_high) / 2)

    # The task is considered to be successful if the knife is inside the box bounding box
    success = reward_in
